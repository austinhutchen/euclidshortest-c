...
void Graph::removeEdge(int key1, int key2) {
  ...
  // Ex: say you need to remove ith index item from a vector "myvector", you can
  // use myvector.erase(myvector.begin()+i)
...
// Check if an edge connecting vertices with keys: key1 and key2 is a bridge of
// the graph, returns true if it is indeed a bridge otherwise false
...
bool Graph::isBridge(int key1, int key2) {
  ...
  // TODO Step1: Get initial_components. Complete and use the DFTraversal
  // function.
  for (int i = 0; i < vertices.size(); i++) {
  ...
  cout << "num. of connected components before removal: " << initial_components
       << endl;

  // TODO Step 2: Remove the edge (key1,key2) from the graph, modify the
  // adjacency list(s) in the removeEdge function.
  ...
  // num. of connected componenets in the modified graph (i.e after removing the
  // edge)
  ...
  cout << "num. of connected components after removal: "
       << components_after_removal << endl;

  // Step4: Add the edge back to the graph
    addEdge(key1, key2);

  // TODO Step5: check if the no of connected components increases after
  // removing the edge and return true
